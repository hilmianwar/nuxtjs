...
const props = defineProps(["toggleEditForm", "menuId"]); // Define props untuk mengakses menuId
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(
      `https://api.mudoapi.tech/menu/${props.menuId}`,
      payload,
      config
    );
    props.toggleEditForm(); // Panggil toggleEditForm dari props
  } catch (error) {
    console.log(error);
  }
};
